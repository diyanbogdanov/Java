...
public class GradeBook {
    ...
    //set array with grades
    public void setGrades(int someGrades[][]) {
        grades = new int[someGrades.length][];
        for (int i = 0; i < someGrades.length; i++) {
            grades[i] = new int[someGrades[i].length];
            for (int j = 0; j < someGrades[i].length; j++) {
                grades[i][j] = someGrades[i][j];
            }
        }
    }
    ...
    // method to return average grades of students
    public double[] getAllAverage() {
        double[] allAverage = new double[grades.length];
        for (int i = 0; i < allAverage.length; i++) {
            allAverage[i] = getAverage(grades[i]);
        }
        return allAverage;
    }
    
    // method to return average grades of studenst above some value
    public double[] filterAverages(double[] averages, double aboveLevel) {
        int counter = 0;
        double[] filterAverages;
        double grade;
        
        for (int i = 0; i < averages.length; i++) {
            if (getAverage(grades[i]) >= aboveLevel) {
                counter++;
            }
        }
        
        filterAverages = new double[counter];
        
        for (int i = averages.length - 1; i >= 0; i--) {
            grade = getAverage(grades[i]);
            if (grade >= aboveLevel) {
                counter--;
                filterAverages[counter] = grade;
            }
        }
